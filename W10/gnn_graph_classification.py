...
class SimpleGNN(torch.nn.Module):
    ...
    def __init__(self, node_feature_dim, state_dim, num_message_passing_rounds):
        ...
        self.input_net = torch.nn.Sequential(
            ...
            torch.nn.ReLU(),
            # torch.nn.Dropout(p=0.1)  # Add dropout to input network
        )
        ...
        self.message_net = torch.nn.ModuleList([
            torch.nn.Sequential(
                ...
                torch.nn.ReLU(),
                torch.nn.Dropout(p=0.4)  # Add dropout to message networks
        ...
        self.update_net = torch.nn.ModuleList([
            torch.nn.Sequential(
                torch.nn.Linear(self.state_dim, self.state_dim),
                torch.nn.ReLU(),
                torch.nn.Dropout(p=0.3)  # Add dropout to update networks
            ) for _ in range(num_message_passing_rounds)])

        # Update network with GRU
    ...
    def forward(self, x, edge_index, batch):
        ...
        for r in range(self.num_message_passing_rounds):
            ...
            msg = self.update_net[r](aggregated)
