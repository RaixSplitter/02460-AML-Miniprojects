...
class EnsembleVAE(nn.Module):
    """
    Define an ensemble of VAE models.
    """

    def __init__(self, prior, decoder, encoder, num_decoders):
        """
        Parameters:
        prior: [torch.nn.Module]
           The prior distribution over the latent space.
    ...
    def mean_elbo(self, x):
        """
        Compute the mean ELBO loss for an ensemble of decoders.
        """

        elbos = []
        for decoder in self.decoders:
            elbos.append(self.elbo(decoder, x))
    ...
    def sample(self, n_samples=1, decoder_idx=None):
        ...
        if (
            decoder_idx is None
        ):  # Randomly select a decoder from the ensemble from an uniform distribution.
            decoder_idx = random.randint(0, len(self.decoders) - 1)
...
if __name__ == "__main__":
    ...
    if args.mode == "train":
        ...
        with torch.no_grad():
            ...
            recon = torch.stack([decoder(model.encoder(data).mean).mean for decoder in model.decoders])
            print(recon.shape)
            recon = recon.mean(0)
            print(recon.shape)
